...
OUTPUT_FILE = BASE_DIR / "kaufland.txt"
PROGRESS_FILE = BASE_DIR / "kaufland_progress.json"
...
def extract_store_record_from_item(item) -> str | None:
	"""Build an output row from one store list item."""
	try:
		street = normalize_spaces(item.locator(".m-store-list__address-street").inner_text())
		city = normalize_spaces(item.locator(".m-store-list__address-city").inner_text())
		lat = normalize_spaces(item.get_attribute("data-lat") or "")
		lng = normalize_spaces(item.get_attribute("data-lng") or "")
	except Exception:
		return None

	if not street or not city or not lat or not lng:
		return None

	address = normalize_spaces(f"{street}, {city}")
...
def get_search_entry_locator(page):
	return page.locator(
		'input#store-search, '
		'input.a-input__field.pac-target-input[placeholder="Bitte gebe einen Standort ein"], '
		'input[placeholder*="PLZ"], input[placeholder*="Ort"]'
	).first


def get_search_button_locator(page):
	"""Return the submit button used to trigger the store search."""
	return page.get_by_role("button", name="Suchen").first
...
def collect_city_matches_from_dialog(page, city: str) -> list[str]:
	...
	store_items = page.locator("li.m-store-list__item")

	def store_items_ready() -> bool:
		try:
			return store_items.count() > 0
		except Exception:
			return False

	wait_until(store_items_ready, f"城市 {city} 的地址列表可见")
	...
	for address in address_candidates:
		city_match = re.search(r",\s*\d{5}\s+(.+?),\s*[-\d\.]+,\s*[-\d\.]+$", address)
		if not city_match:
			continue

		city_from_address = normalize_spaces(city_match.group(1)).split("/")[0].strip()
...
def main() -> None:
	...
	with sync_playwright() as p:
		...
		for index, city in enumerate(cities, start=1):
			...
			search_button = get_search_button_locator(page)

			def search_button_visible_for_city() -> bool:
				return search_button.is_visible()

			wait_until(search_button_visible_for_city, f"城市 {city} 的搜索按钮可见")
			search_button.click()
			random_pause(1.2, 2.2)
