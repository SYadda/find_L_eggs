...
OUTPUT_FILE = BASE_DIR / "aldi_sued.txt"
PROGRESS_FILE = BASE_DIR / "aldi_sued_progress.json"
...
def extract_store_record_from_item(item) -> str | None:
	"""Extract one store address row from a result list item."""
	try:
		address = normalize_spaces(item.locator("p.base-merchant__address").first.inner_text())
	except Exception:
		return None
...
def get_search_entry_locator(page):
	return page.locator(
		'input#merchant-zip-code-input, '
		'input[name="merchant-zip-code-input"], '
...
def collect_city_matches_from_dialog(page, city: str) -> list[str]:
	...
	store_items = page.locator("#tabpanel-walk-in li")
