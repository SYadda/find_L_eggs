...
BASE_URL = "https://www.aldi-nord.de/filialen-und-oeffnungszeiten.html"
BASE_DIR = Path(__file__).resolve().parent
ROOT_DIR = BASE_DIR.parent
CITY_FILE = ROOT_DIR / "city_name.txt"
OUTPUT_FILE = BASE_DIR / "aldi_nord.txt"
PROGRESS_FILE = BASE_DIR / "aldi_nord_progress.json"
...
def extract_city_from_address(address: str) -> str | None:
	...
	- Am Lahof 1 28832 Achim
	"""
	normalized = normalize_spaces(address)
	match = re.search(r"(?:,\s*)?\d{5}\s+(.+)$", normalized)
	if not match:
		return None
...
def ensure_comma_before_zip(address: str) -> str:
	"""Ensure address format is 'prefix, ZIP city' where prefix is everything before ZIP."""
	normalized = normalize_spaces(address)
	zip_match = re.search(r"\b\d{5}\b", normalized)
	if not zip_match:
		return normalized

	prefix = normalize_spaces(normalized[: zip_match.start()].rstrip(","))
	zip_and_city = normalize_spaces(normalized[zip_match.start():])
	if not prefix or not zip_and_city:
		return normalized

	return f"{prefix}, {zip_and_city}"


def extract_store_record_from_item(item) -> str | None:
	"""Extract one store address row from an address span node."""
	try:
		# New cards render address with line breaks inside a span.
		address = normalize_spaces(item.inner_text())
	except Exception:
		return None

	if not address:
		return None
	return ensure_comma_before_zip(address)
...
def get_search_entry_locator(page):
	# The page has multiple search inputs. Use the exact store-locator filter input path first.
	exact_css = (
		"#content > div > div:nth-child(3) > div > "
		"div.store-locator__wrapper > div > "
		"div.store-locator__wrapper__section__search-filter > "
		"div.search-filter > div.search-box > input"
	)
	exact_xpath = (
		'//*[@id="content"]/div/div[3]/div/div[1]/div/div[1]/div[1]/div[1]/input'
	)

	css_locator = page.locator(exact_css)
	if css_locator.count() > 0:
		return css_locator.first

	xpath_locator = page.locator(f"xpath={exact_xpath}")
	if xpath_locator.count() > 0:
		return xpath_locator.first

	# Last-resort fallback within the search-filter area only.
	return page.locator(
		"div.store-locator__wrapper__section__search-filter "
		"div.search-box input[data-testid='store-locator-search-filter-search-box-input-field']"
	).first
...
def has_no_results_overlay(page) -> bool:
	"""Detect the visible 'Kein Ergebnis' overlay in the store card section."""
	no_results_nodes = [
		page.locator(
			"#storeCardSection > div.heading.heading--small."
			"store-locator__wrapper__section__search-filter__store-list__overlay__no-results-heading"
		).first,
		page.locator("xpath=//*[@id='storeCardSection']/div[1]").first,
	]

	for node in no_results_nodes:
		try:
			if not node.is_visible():
				continue
			text = normalize_spaces(node.inner_text())
			if text.casefold() == "kein ergebnis":
				return True
		except Exception:
			continue
...
def click_city_suggestion_by_exact_text(page, city: str) -> bool:
	"""After typing a city, click exact-matched item; fallback to first item when no exact match."""
	target_city = normalize_spaces(city).casefold()
	main_text_nodes = page.locator(
		"#content > div > div:nth-child(3) > div > "
		"div.store-locator__wrapper > div > "
		"div.store-locator__wrapper__section__search-filter > "
		"div.search-filter > div.search-filter__result > ul > li > div > span.text.text--bold, "
		"span[data-testid^='search-filter-result-list-item-'][data-testid$='-main-text']"
	)
	attempt = 0
	while True:
		attempt += 1

		if has_no_results_overlay(page):
			print(f"城市 {city}: 出现 'Kein Ergebnis'，跳过")
			return False

		try:
			count = main_text_nodes.count()
		except Exception:
			count = 0

		if count <= 0:
			if attempt == 1 or attempt % 4 == 0:
				print(f"城市 {city}: 等待下拉结果出现")
			time.sleep(1.0)
			continue

		for i in range(count):
			try:
				text = normalize_spaces(main_text_nodes.nth(i).inner_text())
			except Exception:
				continue

			if text.casefold() != target_city:
				continue

			main_text_nodes.nth(i).locator("xpath=ancestor::li[1]").click()
			random_pause(1.0, 1.8)
			return True

		# No exact match found: fallback to first suggestion as requested.
		main_text_nodes.first.locator("xpath=ancestor::li[1]").click()
		random_pause(1.0, 1.8)
		print(f"城市 {city}: 未找到完全匹配项，已点击下拉首项")
		return True
...
def collect_city_matches_from_dialog(page, city: str) -> list[str]:
	...
	store_items = page.locator(
		"#storeCardSection div.address-card__wrapper__section__address > span"
	)
	...
	while True:
		...
		if has_no_results_overlay(page):
			print(f"城市 {city}: 地址区显示 'Kein Ergebnis'，返回空结果")
			return []

		try:
			if store_items.count() > 0:
				break
		except Exception:
			pass
		...
		if attempt == 1 or attempt % 4 == 0:
			print(
				f"等待页面满足条件: 城市 {city} 的地址列表可见。"
				"如果出现 Cookie 通知或验证码，请手动处理，脚本会自动继续..."
			)
		time.sleep(1.0)

	address_candidates: list[str] = []
	for i in range(store_items.count()):
		item = store_items.nth(i)
		record = extract_store_record_from_item(item)
		if record:
			address_candidates.append(record)
...
def main() -> None:
	...
	with sync_playwright() as p:
		...
		for index, city in enumerate(cities, start=1):
			...
			random_pause(1.8, 3.0)

			# The store list only appears after selecting the exact-matched city suggestion.
			selected = click_city_suggestion_by_exact_text(page, city_search_term)
			if not selected:
				processed_cities.add(city_key)
				save_processed_cities(processed_cities)
				print(f"[{index}/{len(cities)}] {city} (搜索词: {city_search_term}): 无结果，跳过")
